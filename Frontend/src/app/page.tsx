    ...
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      ...
      <style jsx>{`
        @keyframes float {
          ...
          25% { transform: translateY(-400px) translateX(20px); }
          50% { transform: translateY(-800px) translateX(-20px); }
          75% { transform: translateY(-1200px) translateX(15px); }
          100% { transform: translateY(-1600px) translateX(0); }
        }
      `}</style>
    </div>
  );
...
const HeroSection = () => {
  ...
  return (
    <section className="relative flex items-center justify-center min-h-screen px-4 py-20 text-center lg:text-left bg-gradient-to-br from-slate-50 via-white to-indigo-50 overflow-hidden">
      <FloatingParticles />
      
      <div className="container mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-center relative z-10">
        {/* Columna de Texto */}
        <div className="flex flex-col items-center lg:items-start space-y-6">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-indigo-100 rounded-full text-indigo-600 text-sm font-semibold animate-pulse">
            <Sparkles size={16} />
          ...
          <p className={`${lato.className} text-lg md:text-xl text-slate-600 max-w-xl leading-relaxed`}>
            Ahorra horas de lectura y acelera tu aprendizaje. Genera resúmenes y notas de estudio con un solo clic.
          </p>
          
          <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
            <a href="/summarizer" className="group px-8 py-4 bg-indigo-600 text-white font-semibold rounded-lg shadow-lg hover:shadow-xl hover:bg-indigo-700 transition-all duration-300 transform hover:scale-105 flex items-center justify-center gap-2">
              Genera tu primer resumen
              <ArrowRight size={20} className="group-hover:translate-x-1 transition-transform" />
            </a>
            <a href="#demoSection" className="px-8 py-4 bg-white text-indigo-600 font-semibold rounded-lg border-2 border-indigo-600 hover:bg-indigo-50 transition-all duration-300">
              Ver demo
            </a>
          </div>
          ...
          <div className="flex gap-8 pt-8 border-t border-slate-200 w-full">
            ...
            <div>
              <div className="text-3xl font-bold text-slate-800">1min</div>
        ...
        <div className="relative h-[500px] w-full flex items-center justify-center">
          <div className="relative w-full h-full">
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="w-80 h-80 bg-gradient-to-br from-indigo-200 to-purple-200 rounded-full opacity-20 animate-pulse" />
            </div>

            {/* Documento Original */}
            <div className={`absolute top-16 left-8 transition-all duration-700 ${
              animationState === 'idle' ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-8'
            }`}>
              <div className="w-48 h-64 bg-white rounded-lg shadow-xl p-4 border-2 border-slate-200">
                <div className="flex items-center gap-2 mb-3">
                  <FileText size={20} className="text-slate-400" />
                  <div className="text-xs text-slate-600 font-semibold">Documento.pdf</div>
                </div>
                <div className="space-y-2">
                  {[...Array(8)].map((_, i) => (
                    <div key={i} className="h-2 bg-slate-200 rounded" style={{ width: `${Math.random() * 40 + 60}%` }} />
                  ))}
                </div>
              </div>
            </div>
            ...
            <div className={`absolute inset-0 flex flex-col items-center justify-center transition-all duration-500 ${
              animationState === 'processing' ? 'opacity-100 scale-100' : 'opacity-0 scale-75'
            }`}>
              <div className="relative">
                <Cpu size={80} className="text-indigo-500 animate-spin" style={{ animationDuration: '3s' }} />
                <div className="absolute inset-0 flex items-center justify-center">
                  {/* <Sparkles size={40} className="text-purple-500 animate-pulse" /> */}
                </div>
              </div>
              <div className="mt-6 w-64">
                <div className="text-sm text-slate-600 font-semibold mb-2">Procesando con IA...</div>
            ...
            {/* Resumen Generado */}
            <div className={`absolute top-12 right-8 transition-all duration-700 ${
              animationState === 'summarized' ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-8'
            }`}>
              <div className="w-56 bg-white rounded-lg shadow-2xl p-5 border-2 border-indigo-200">
                <div className="flex items-center gap-2 mb-3">
                  <BookOpenCheck size={20} className="text-indigo-500" />
                  <div className="text-xs text-slate-700 font-bold">Resumen</div>
                  <Download size={16} className="ml-auto text-slate-400 cursor-pointer hover:text-indigo-500" />
                </div>
                <div className="space-y-3 text-left">
                  <div>
                    <div className="text-xs font-bold text-slate-700 mb-1">💡 Idea Principal</div>
                    <div className="text-xs text-slate-600 leading-relaxed">La innovación tecnológica transforma...</div>
                  </div>
                  <div>
                    <div className="text-xs font-bold text-slate-700 mb-1">📊 Datos Clave</div>
                    <div className="text-xs text-slate-600">• Incremento del 45%</div>
                    <div className="text-xs text-slate-600">• 3 factores principales</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className={`absolute inset-0 transition-all duration-700 ${
              ...
              <div className="relative w-full h-full">
                <div className="absolute top-20 left-12 w-40 h-40 bg-yellow-200 rounded-lg shadow-lg p-3 transform -rotate-6 hover:rotate-0 transition-transform cursor-pointer">
                  <div className="text-xs font-bold text-slate-700 mb-2">🔑 Concepto A</div>
                  <div className="text-xs text-slate-600">Definición fundamental del tema principal</div>
                </div>
                <div className="absolute top-32 right-16 w-40 h-40 bg-blue-200 rounded-lg shadow-lg p-3 transform rotate-3 hover:rotate-0 transition-transform cursor-pointer">
                  <div className="text-xs font-bold text-slate-700 mb-2">📌 Idea B</div>
                  <div className="text-xs text-slate-600">Punto importante a recordar</div>
                </div>
                <div className="absolute bottom-24 left-20 w-40 h-40 bg-green-200 rounded-lg shadow-lg p-3 transform rotate-6 hover:rotate-0 transition-transform cursor-pointer">
                  <div className="text-xs font-bold text-slate-700 mb-2">✨ Conclusión</div>
                  <div className="text-xs text-slate-600">Resumen final del documento</div>
                </div>
              </div>
            </div>
          </div>
    ...
    <section id="how-it-works" className="py-24 bg-white relative overflow-hidden">
      ...
      <div ref={ref} className="container mx-auto px-4 text-center relative z-10">
        <div className={`transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <h2 className={`${poppins.className} text-3xl md:text-4xl lg:text-5xl font-bold text-slate-800 mb-4`}>
            Empieza en segundos
          </h2>
          ...
          {steps.map((step, index) => (
            <div key={index} className="relative">
              ...
              {index < steps.length - 1 && (
                <div className="hidden md:block absolute top-16 left-1/2 w-full h-0.5 bg-gradient-to-r from-slate-200 to-transparent z-0" />
              )}
              
              <div 
                className={`relative bg-white p-8 rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-500 border-2 border-slate-100 hover:border-indigo-200 group ${
                  isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
                }`}
                style={{ transitionDelay: `${index * 200}ms` }}
              >
                {/* Número del paso */}
                <div className="absolute -top-4 -right-4 w-10 h-10 bg-white/20 backdrop-blur-3xl border-indigo-200 border text-slate-800 rounded-full flex items-center justify-center font-bold shadow-lg">
                  {index + 1}
                </div>
                ...
                <h3 className={`${poppins.className} text-xl font-bold text-slate-800 mb-3`}>
                  {step.title}
                </h3>
                <p className={`${lato.className} text-slate-600 leading-relaxed`}>
                  {step.description}
                </p>
              </div>
            </div>
...
const FeaturesSection = () => {
  ...
  return (
    <section className="py-24 bg-gradient-to-br from-slate-50 to-indigo-50 overflow-hidden">
      <div className="container mx-auto px-4">
        <div className="text-center mb-16">
          ...
          <p className="text-lg text-slate-600 max-w-2xl mx-auto">
            Herramientas potentes diseñadas para maximizar tu eficiencia de aprendizaje
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-7xl mx-auto">
          ...
          <div 
            ref={ref1} 
            className={`group bg-white p-8 rounded-3xl shadow-xl hover:shadow-2xl transition-all duration-700 border-2 border-slate-100 hover:border-indigo-300 ${
          ...
          >
            <div className="flex items-center gap-4 mb-6">
              ...
              <h3 className={`${poppins.className} text-2xl font-bold text-slate-800`}>
                Summarizer
              </h3>
            </div>
            
            <p className={`${lato.className} text-slate-600 mb-6 text-lg leading-relaxed`}>
            ...
            <div className="relative h-52 bg-gradient-to-br from-slate-50 to-indigo-50 rounded-xl border-2 border-slate-200 p-6 overflow-hidden group-hover:border-indigo-300 transition-colors">
              <div className="absolute top-4 left-4 w-8 h-1 bg-slate-300 rounded" />
              <div className="absolute top-8 left-4 right-4">
                <div className="space-y-2">
                  {[...Array(6)].map((_, i) => (
                    <div key={i} className="h-2 bg-slate-200 rounded animate-pulse" style={{ width: `${90 - i * 10}%`, animationDelay: `${i * 100}ms` }} />
                  ))}
                </div>
              </div>
          ...
          <div 
            ref={ref2} 
            className={`group bg-white p-8 rounded-3xl shadow-xl hover:shadow-2xl transition-all duration-700 border-2 border-slate-100 hover:border-purple-300 ${
          ...
          >
            ...
            <div className="relative h-52 bg-gradient-to-br from-slate-50 to-purple-50 rounded-xl border-2 border-slate-200 p-4 overflow-hidden group-hover:border-purple-300 transition-colors">
...
const DemoSection = () => {
  ...
  return (
    <section id="demoSection" className="py-24 bg-gradient-to-br from-slate-900 via-slate-800 to-indigo-900 relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 text-center relative z-10">
        <div className="mb-12">
          <h2 className={`${poppins.className} text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-4`}>
            La magia en acción
          </h2>
          <p className="text-lg text-slate-300 max-w-2xl mx-auto">
            Prueba nuestra demo interactiva y ve el poder de la IA en tiempo real
          </p>
        </div>
        ...
        <div className="max-w-4xl mx-auto min-h-[450px] bg-slate-900/80 backdrop-blur-sm rounded-3xl border-2 border-slate-700 shadow-2xl flex flex-col items-center justify-center p-8 transition-all duration-500">
            ...
            <div className="text-center space-y-6">
              ...
              >
                ...
                <div className="relative bg-slate-800 p-8 rounded-2xl border-2 border-slate-600 group-hover:border-indigo-400 transition-colors">
                  <UploadCloud size={64} className="text-slate-400 group-hover:text-white transition-colors mx-auto mb-4"/>
          ...
          {step === 'uploaded' && (
            <div className="text-center space-y-6 animate-in">
              ...
              <p className="text-slate-300 mb-6">¿Qué quieres hacer con este documento?</p>
          ...
          {(step === 'summarizing' || step === 'quicknoting') && (
            <div className="w-full max-w-2xl">
              <div className={`transition-all duration-700 ${showResult ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
                {step === 'summarizing' && (
                  <div className="bg-slate-800/50 backdrop-blur rounded-2xl p-8 border-2 border-indigo-500/50">
                    <div className="flex items-center justify-between mb-6">
                      <div className="flex items-center gap-3">
                        ...
                        <h4 className="text-white font-bold text-xl">Resumen Generado</h4>
                      </div>
                      <Download size={20} className="text-slate-400 hover:text-white cursor-pointer transition-colors" />
                    </div>
                    <div className="space-y-4 text-left">
                      <div className="bg-slate-700/50 rounded-lg p-4">
                        ...
                        <p className="text-slate-200 text-sm leading-relaxed">
                          El documento explora los fundamentos de la inteligencia artificial moderna, destacando el impacto del aprendizaje profundo en diversas industrias.
                        </p>
                      </div>
                      <div className="bg-slate-700/50 rounded-lg p-4">
                        ...
                        <ul className="text-slate-200 text-sm space-y-1">
                          <li>• Incremento del 45% en adopción de IA (2020-2024)</li>
                          <li>• 3 factores principales: computación, datos y algoritmos</li>
                          <li>• Inversión global de $500B proyectada para 2025</li>
                        </ul>
                      </div>
                      <div className="bg-slate-700/50 rounded-lg p-4">
                        ...
                        <p className="text-slate-200 text-sm leading-relaxed">
                          La IA transformará significativamente el futuro del trabajo, requiriendo adaptación continua y nuevas habilidades.
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-6">
                      <div className="flex items-center gap-3">
                        ...
                        <h4 className="text-white font-bold text-xl">Notas Rápidas Generadas</h4>
                      </div>
                      <Download size={20} className="text-slate-400 hover:text-white cursor-pointer transition-colors" />
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div className="bg-yellow-200 text-slate-800 rounded-xl p-4 shadow-lg transform hover:scale-105 transition-transform cursor-pointer">
                        ...
                        <p className="text-sm">
                          <strong>Aprendizaje Profundo:</strong> Redes neuronales con múltiples capas que aprenden representaciones jerárquicas de datos.
                        </p>
                      </div>
                      <div className="bg-blue-200 text-slate-800 rounded-xl p-4 shadow-lg transform hover:scale-105 transition-transform cursor-pointer">
                        <div className="font-bold mb-2 flex items-center gap-2">
                          <span className="text-lg">📌</span> Definición Importante
                        </div>
                        <p className="text-sm">
                          <strong>Machine Learning:</strong> Sistemas que mejoran automáticamente con la experiencia sin ser programados explícitamente.
                        </p>
                      </div>
                      <div className="bg-green-200 text-slate-800 rounded-xl p-4 shadow-lg transform hover:scale-105 transition-transform cursor-pointer">
                        ...
                        <p className="text-sm">
                          Los datos son el combustible de la IA moderna. Más datos = mejores modelos.
                        </p>
                      </div>
                      <div className="bg-purple-200 text-slate-800 rounded-xl p-4 shadow-lg transform hover:scale-105 transition-transform cursor-pointer">
                        ...
                        <p className="text-sm">
                          GPT-4 utiliza 1.76 billones de parámetros para generar texto coherente y contextual.
                        </p>
                      </div>
                    </div>
                  </div>
              ...
              <button 
                onClick={() => { setStep('idle'); setShowResult(false); }} 
        ...
        <p className="mt-8 text-slate-400 text-sm">
          Esta es solo una demostración. Los resultados reales pueden variar según el contenido del documento.
        </p>
      </div>
    </section>
  );
    ...
    <section id="testimonials" className="py-24 bg-white relative overflow-hidden">
      ...
      <div ref={ref} className="container mx-auto px-4 relative z-10">
        <div className={`text-center mb-16 transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <p className="text-lg text-slate-600 max-w-2xl mx-auto">
            Miles de estudiantes ya confían en StudyApp para mejorar su aprendizaje
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {testimonials.map((testimonial, index) => (
            <div 
              key={index}
              className={`bg-white p-8 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-500 border-2 border-slate-100 hover:border-indigo-200 ${
                isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
              }`}
            ...
            >
              ...
              <p className={`${lato.className} text-slate-700 italic leading-relaxed`}>
                &quot;{testimonial.text}&quot;
              </p>
...
const FinalCTASection = () => {
  ...
  return (
    <section className="py-24 bg-white relative overflow-hidden">
      ...
      <div ref={ref} className="container mx-auto px-4 text-center max-w-4xl relative z-10">
        <div className={`transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          <div className="inline-block px-4 py-2 bg-indigo-100 text-indigo-600 rounded-full text-sm font-semibold mb-4">
            Comienza ahora
          </div>
          <h2 className={`${poppins.className} text-3xl md:text-4xl lg:text-5xl font-bold text-slate-800 mb-6`}>
            Transforma tu forma de estudiar hoy
          </h2>
          <p className={`${lato.className} text-xl text-slate-600 mb-10 max-w-2xl mx-auto leading-relaxed`}>
            Únete a miles de estudiantes que ya están aprendiendo de forma más inteligente. 
            Tu primer resumen está a solo un clic de distancia.
          </p>
          
          <div className="flex flex-col sm:flex-row gap-4 justify-center mb-12">
            <a href="/summarizer" className="group px-10 py-5 bg-gradient-to-r from-indigo-400 to-purple-400 hover:from-indigo-500 hover:to-purple-500 text-white font-bold rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105 text-lg flex items-center justify-center gap-2">
              Comenzar gratis
              <ArrowRight size={20} className="group-hover:translate-x-1 transition-transform" />
            </a>
          </div>
